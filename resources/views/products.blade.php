...
<div>
    <div class="w-screen flex flex-col items-center justify-center">
        ...
        <table class="table-auto text-zinc-50 font-medium mt-8 rounded">
            <thead class="pl-6 font-semibold text-sm text-left pr-3 py-3.5 text-sky-500 bg-slate-100 dark:bg-slate-800 rounded">
                <tr class="table-row">
                    <!-- <th class="w-fit capitalize pl-2 pr-6 py-3 whitespace-nowrap" scope="col">ID</th> -->
                    <th class="w-fit capitalize pl-2 pr-6 py-3 whitespace-nowrap" scope="col">Imagem</th>
                    <th class="w-fit capitalize pl-2 pr-6 py-3 whitespace-nowrap" scope="col">Nome</th>
                    <th class="w-fit capitalize pl-2 pr-6 py-3 whitespace-nowrap" scope="col">Preço</th>
                    <th class="w-fit capitalize pl-2 pr-6 py-3 whitespace-nowrap" scope="col">Categorias</th>
            ...
            @if($product->active)
            <tr class="even:bg-slate-100 odd:text-slate-950 even:bg-slate-950/15 *:w-fit *:capitalize pl-2 *:pr-6 *:py-3 *:whitespace-nowrap">
                @else
            <tr class=" *:w-fit *:capitalize pl-2 *:pr-6 *:py-3 *:whitespace-nowrap bg-neutral-100 line-through text-slate-950">
                @endif
                <td>
                    @if($product->url_image)
                    <img src="{{ asset(Storage::url($product->url_image)) }}" alt="{{ $product->name }}" class="rounded max-w-16">
                    @endif
                </td>
                ...
                <td class="flex pl-2 pr-1 w-fit py-5 flex-nowrap gap-x-2">
                    <form method="POST" action="{{ route('produtodelete', ['id'=> $product->id]) }}">
                        <input type="hidden" name="_method" value="DELETE">
                        {{ csrf_field()}}
                        <a href="{{ route('produtoform', ['id' => $product->id]) }}" class="bg-emerald-500 rounded-full py-1 px-4 focus:bg-slate-50
